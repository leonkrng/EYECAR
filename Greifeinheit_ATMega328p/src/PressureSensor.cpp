...
void PressureSensor::SetupSensor() { // scalingfactor is known
   ...
   Serial.print("measured value after tare - should be around 0: ");
   Serial.println(hx711.get_units(10));

   // 3. setup step
   // set scalefactor = -112
   // pre determined 
   hx711.set_scale(fScalingFactorGramms); // set scalingfactor
...
bool PressureSensor::MovementIsSafe() {
    ...
    if (fTmpWeight <= fMaximumPushingWeight) { // when measured value is lower or equal to treshold
